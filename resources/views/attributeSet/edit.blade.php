...
@section('title','编辑产品类型')
@section('head')
<script src="{{ asset('js/attribute.js') }}"></script>
<script>
$(function() {
	var oAttrIds = [{{ $oAttributeIds }}];
	var oEntityIds = [{{ $oEntityIds }}];
	for (var i = 0; i < oAttrIds.length; i ++) {
		moveToSelected(oAttrIds[i], oEntityIds[i]);
	}
	addAjaxListener("#submitButton", "#attributeSetEdit");
});
</script>
@endsection
@section('content')
	<div class="col-lg-9">
		<div class="panel panel-success">
			...
			<form method="post" id="attributeSetEdit" class="form-horizontal" action="{{ url('attribute/set/update') }}" data-href="{{ url('attribute/set') }}">
				{!! csrf_field() !!}
				...
				<div class="form-group">
					...
					<div class="col-md-10">
						<input type="text" name="name" id="name" class="form-control" value="{{ $attributeSet->name }}" placeholder="64位内">
					</div>
				</div>
				<hr />
